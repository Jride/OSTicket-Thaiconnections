...
<div><p id="msg_info"><i class="icon-info-sign"></i>&nbsp; Search existing customers or add a new customer.</p></div>
...
<form method="post" class="user" action="<?php echo $info['action'] ?  $info['action'] : '#users/lookup'; ?>">
    ...
    <a class="action-button pull-right" style="overflow:inherit"
        id="unselect-user"  href="#"><i class="icon-remove"></i> Add New Customer</a>
<?php if ($user) { ?>
...
<form method="post" class="user" action="<?php echo $info['action'] ?: '#users/lookup/form'; ?>">
    ...
    <?php
        ...
        $form->render(true, 'Create New Customer'); ?>
    </table>
    <hr>
    <p class="full-width">
        <span class="buttons" style="float:left">

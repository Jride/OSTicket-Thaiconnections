...
class UsersAjaxAPI extends AjaxController {
    ...
    function register($id) {
        ...
        if ($_POST) {
            ...
            if ($user->getAccount())
                $info['error'] = 'Customer already registered';
            ...
            if ($errors['err'])
                ...
                $info['error'] = 'Unable to register customer - try again!';
        }
        ...
        if ($_POST) {
            if ($user->tickets->count()) {
                if (!$thisstaff->canDeleteTickets()) {
                    $info['error'] = 'You do not have permission to delete a customer with tickets!';
                } elseif ($_POST['deletetickets']) {
                    foreach($user->tickets as $ticket)
                        $ticket->delete();
                } else {
                    $info['error'] = 'You cannot delete a customer with tickets!';
                }
            }

            if (!$info['error'] && $user->delete())
                 Http::response(204, 'Customer deleted successfully');
            elseif (!$info['error'])
                $info['error'] = 'Unable to delete customer - try again!';
        }
    ...
    function getUser($id=false) {
        ...
        $info = array('error' =>'Unknown or invalid customer');
    ...
    function selectUser($id) {
        ...
        $info = array('title' => 'Select Customer');
    ...
    static function _lookupform($form=null, $info=array()) {
        ...
        if (!$info or !$info['title'])
            $info += array('title' => 'Lookup or create a customer');

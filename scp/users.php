...
if ($_REQUEST['id'] && !($user=User::lookup($_REQUEST['id'])))
    $errors['err'] = 'Unknown or invalid custumer ID.';

if ($_POST) {
    switch(strtolower($_REQUEST['do'])) {
        case 'update':
            if (!$user) {
                $errors['err']='Unknown or invalid customer.';
            } elseif(($acct = $user->getAccount())
                    && !$acct->update($_POST, $errors)) {
                 $errors['err']='Unable to update customer account information';
            } elseif($user->updateInfo($_POST, $errors)) {
                $msg='Customer updated successfully';
                $_REQUEST['a'] = null;
            } elseif(!$errors['err']) {
                $errors['err']='Unable to update customer profile. Correct any error(s) below and try again!';
            }
            break;
        case 'create':
            ...
            if (($user = User::fromForm($form))) {
                ...
                $errors['err'] = 'Unable to add customer. Correct any error(s) below and try again.';
            }
            break;
        case 'confirmlink':
            if (!$user || !$user->getAccount())
                $errors['err'] = 'Unknown or invalid customer account';
        ...
        case 'mass_process':
            if (!$_POST['ids'] || !is_array($_POST['ids']) || !count($_POST['ids'])) {
                $errors['err'] = 'You must select at least one customer member.';
            } else {
